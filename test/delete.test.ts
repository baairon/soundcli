...
async function makeTree(): Promise<string> {
  ...
  await fs.mkdir(path.join(root, "SoundCloud", "flume", "Liked Songs"), {
    recursive: true,
  });
  await fs.writeFile(path.join(root, "YouTube", "Singles", "a.m4a"), "a");
  await fs.writeFile(
    path.join(root, "SoundCloud", "flume", "Liked Songs", "b.m4a"),
    "b",
  );
  await fs.writeFile(
    path.join(root, "SoundCloud", "flume", "Liked Songs", "c.m4a"),
    "c",
  );
  return root;
}
...
describe("deleteTracks", () => {
  ...
  it("keeps folders that still hold songs, then prunes once emptied", async () => {
    ...
    try {
      const lib = new FakeLibrary();
      const dir = path.join(root, "SoundCloud", "flume", "Liked Songs");
